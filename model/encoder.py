...
class LayerNorm(nn.Module):
    ...
    def forward(self, x):
        ...
        return self.a_2 * (x - mean) / (std + self.eps) + self.b_2 # 每个特征设置一个反归一化的均值与方差、广播
...
class MultiHeadedAttention(nn.Module):
    ...
    def forward(self, query, key, value, mask=None):
        ...
        query, key, value = [l(x).view(batch_size, -1, self.h, self.d_k).transpose(1, 2)
                            for l, x in zip(self.linear_layers, (query, key, value))] # x: (batch_size, seq_len, d_model) -> (batch_size, n_head, seq_len, d_k)
...
class Attention(nn.Module):
    ...
    def forward(self, query, key, value, mask=None, dropout=None):
        ...
        # scores = torch.exp(torch.matmul(query, key.transpose(-2, -1))) \ # EXP -> INF -> NAN
        #         / math.sqrt(query.size(-1))
