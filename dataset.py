...
class Dataset:
    ...
    def convertTimes(self):      
        ...
        for split in ["train", "valid", "test"]:
            for i, fact in enumerate(self.data[split]):
                ...
                if self.name == "brooklyn":
                    date_str = time.strftime("%Y-%m-%d", time.localtime(int(fact_date)))
                    date = list(map(float, date_str.split("-")))
                    self.data[split][i] += date # extend
                    self.data[split][i] += [int(fact_date)]
                    # print(self.data[split][i])
                    # assert(1==0)
                else:
    ...
    def nextBatch(self, batch_size, neg_ratio=1):
        ...
        # print(bp_facts.shape, bp_facts)
        batch = shredFacts(self.addNegFacts2(bp_facts, neg_ratio), self.name)
        return batch
